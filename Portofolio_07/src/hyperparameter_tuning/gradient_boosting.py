...
def hyperparameter_tuning(n_trials=10):
    def objective(trial):
        ...
        max_leaves = trial.suggest_int('max_leaves', 1, 2000)
        learning_rate = trial.suggest_float('learning_rate', 1e-4, 0.5)
        reg_alpha = trial.suggest_float('reg_alpha', 0, 100)
        reg_lambda = trial.suggest_float('reg_lambda', 0, 100)
        colsample_bytree = trial.suggest_float('colsample_bytree', 0, 1)
        colsample_bylevel = 1 # trial.suggest_float('colsample_bylevel', 0, 1)
        colsample_bynode = 1 # trial.suggest_float('colsample_bynode', 0, 1)
        ...
        # training
        xgb = XGBRegressor(n_estimators=100, max_depth=max_depth, max_leaves=max_leaves, grow_policy='lossguide',
