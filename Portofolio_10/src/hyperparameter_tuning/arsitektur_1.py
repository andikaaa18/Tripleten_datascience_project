...
def create_model(input_shape, filters, neuron_1, l1_constant, l2_constant):
    ...
    model = Sequential()
    model.add(backbone)
    model.add(Conv2D(filters=filters, kernel_size=(2,2), activation='relu', kernel_regularizer=l1_l2(l1=l1_constant, l2=l2_constant)))
...
def hyperparameter_tuning(n_trials=10):
    def objective(trial):
        # hyperparameter
        filters = trial.suggest_categorical('filters', [1024, 512, 256, 128])
        neuron_1 = trial.suggest_categorical('neuron_1', [512, 256, 128, 64, 32, 16])
        ...
        model = create_model(input_shape=(150,150,3), filters=filters, neuron_1=neuron_1, 
                             l1_constant=l1_constant, l2_constant=l2_constant)
...
if __name__=='__main__':
    hyperparameter_tuning(n_trials=1)