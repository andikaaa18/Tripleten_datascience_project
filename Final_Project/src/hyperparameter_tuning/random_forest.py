...
def hyperparameter_tuning(n_trials=10):
    def objective(trial):
        ...
        min_samples_leaf = trial.suggest_int('min_samples_leaf', 1, min_samples_split-1)
        ...
        # training
        rfc = RandomForestClassifier(n_estimators=100, max_depth=max_depth, min_samples_split=min_samples_split,
                                     criterion=criterion, min_samples_leaf=min_samples_leaf, max_leaf_nodes=max_leaf_node, 
                                     bootstrap=True, class_weight='balanced_subsample', n_jobs=-1, random_state=101010,
                                     max_features=max_features)
        
        rfc.fit(*train_data)
        val_pred_proba = rfc.predict_proba(feature_val)
...
if __name__ == '__main__':
    hyperparameter_tuning(n_trials=1000)
