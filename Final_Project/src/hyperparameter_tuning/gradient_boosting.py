...
def hyperparameter_tuning(n_trials=10):
    def objective(trial):
        ...
        max_leaves = trial.suggest_int('max_leaves', 50, 200)
        ...
        # training
        xgbc = XGBClassifier(n_estimators=100, max_depth=max_depth, max_leaves=max_leaves, learning_rate=learning_rate,
                             reg_alpha=reg_alpha, reg_lambda=reg_lambda, 
                             n_jobs=-1, random_state=101010)
        
        xgbc.fit(*train_data)
        val_pred_proba = xgbc.predict_proba(feature_val)
